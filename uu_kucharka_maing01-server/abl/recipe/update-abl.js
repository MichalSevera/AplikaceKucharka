...
function updateStars(recipe, userId, starred) {
  let stars = recipe.starred ? [...recipe.starred] : [];
  if (starred && !stars.includes(userId)){
    stars.push(userId);
  } else if (!starred && stars.includes(userId)) {
...
async function SetStarAbl(req, res) {
  ...
  try {
    ...
    recipe = await dao.updateRecipe({id: recipe.id, starred: updateStars(recipe, userId, starred)});
    res.json(recipe);
